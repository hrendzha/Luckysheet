...
import { replaceHtml, mouseclickposition } from "../utils/util";
...
let isInitialSheetConfig = false,
  luckysheetcurrentSheetitem = null,
  jfdbclicklagTimeout = null,
  oldSheetFileName = "";
function showsheetconfigmenu() {
  if (!isInitialSheetConfig) {
    ...
    $("#luckysheetsheetconfigcolorur").spectrum({
      ...
      change: function (color) {
        ...
        let oldcolor = null;
        if (
          luckysheetcurrentSheetitem.find(".luckysheet-sheets-item-color")
            .length > 0
        ) {
  ...
  if (
    Store.luckysheetfile[index].color != null &&
    Store.luckysheetfile[index].color.length > 0
  ) {
    $("#luckysheetsheetconfigcolorur").spectrum(
      "set",
      Store.luckysheetfile[index].color
    );
  }

  $("#luckysheetsheetconfigcolorur")
    .parent()
    .find("span, div, button, input, a")
  ...
  setTimeout(function () {
    mouseclickposition(
      ...
      luckysheetcurrentSheetitem.offset().left +
        luckysheetcurrentSheetitem.width(),
      luckysheetcurrentSheetitem.offset().top - 18,
      "leftbottom"
    );
  }, 1);
}
...
let luckysheetsheetrightclick = function ($t, $cur, e) {
  ...
  if (
    $cur.hasClass("luckysheet-sheets-item-name") &&
    $cur.attr("contenteditable") == "true"
  ) {
    return;
  }
  if (
    ...
    setTimeout(function () {
      formula.setCaretPosition(
        formula.rangeSetValueTo.get(0),
        0,
        formula.rangeSetValueTo.text().length
      );
      formula.createRangeHightlight();
      $("#luckysheet-input-box-index")
        .find(".luckysheet-input-box-index-sheettxt")
        .remove()
        .end()
        .prepend(
          "<span class='luckysheet-input-box-index-sheettxt'>" +
            sheetmanage.getSheetName(formula.rangetosheet) +
            "!</span>"
        )
        .show();
  ...
  if (
    $cur.hasClass("luckysheet-sheets-item-menu") ||
    $cur.hasClass("fa-sort-desc") ||
    e.which == "3"
  ) {
...
export function initialSheetBar() {
  ...
  $("#luckysheet-sheet-area")
    .on("mousedown", "div.luckysheet-sheets-item", function (e) {
      if (isEditMode()) {
        // alert("非编辑模式下不允许该操作！");
        return;
      }
      ...
      let $t = $(this),
        $cur = $(e.target),
        $item = $cur.closest(".luckysheet-sheets-item");
      ...
      if (
        $item.hasClass("luckysheet-sheets-item-active") &&
        $item.find(".luckysheet-sheets-item-name").attr("contenteditable") ==
          "false"
      ) {
  ...
  let luckysheetsheetnameeditor = function ($t) {
    if (
      Store.allowEdit === false ||
      !luckysheetConfigsetting.sheetRightClickConfig.rename
    ) {
      return;
    }
  ...
  $("#luckysheet-sheet-area").on(
    "dblclick",
    "span.luckysheet-sheets-item-name",
    function (e) {
  ...
  $("#luckysheetsheetconfigrename").click(function () {
    var $name = luckysheetcurrentSheetitem.find(
      "span.luckysheet-sheets-item-name"
    );
    // 钩子 sheetEditNameBefore
    if (
      !method.createHookFunction("sheetEditNameBefore", {
        i: luckysheetcurrentSheetitem.data("index"),
        name: $name.text(),
      })
    ) {
      return;
    }
    luckysheetsheetnameeditor(
      luckysheetcurrentSheetitem.find("span.luckysheet-sheets-item-name")
    );
    $("#luckysheet-input-box").removeAttr("style");
    $("#luckysheet-sheet-list, #luckysheet-rightclick-sheet-menu").hide();
  });
  ...
  $("#luckysheetsheetconfigmoveleft").click(function () {
    if (luckysheetcurrentSheetitem.prevAll(":visible").length > 0) {
      luckysheetcurrentSheetitem.insertBefore(
  ...
  $("#luckysheetsheetconfigmoveright").click(function () {
    if (luckysheetcurrentSheetitem.nextAll(":visible").length > 0) {
      luckysheetcurrentSheetitem.insertAfter(
  ...
  $("#luckysheetsheetconfigdelete").click(function (e) {
    ...
    if (
      $("#luckysheet-sheet-container-c .luckysheet-sheets-item:visible")
        .length <= 1
    ) {
      if (isEditMode()) {
    ...
    tooltip.confirm(
      locale_sheetconfig.confirmDelete +
        "【" +
        Store.luckysheetfile[index].name +
        "】？",
      "<span style='color:#9e9e9e;font-size:12px;'>" +
        locale_sheetconfig.redoDelete +
        "</span>",
      function () {
  ...
  $("#luckysheetsheetconfighide").click(function () {
    if (
      $("#luckysheet-sheet-area div.luckysheet-sheets-item:visible").length == 1
    ) {
      if (isEditMode()) {
  ...
  $("#luckysheet-sheets-m").click(function (e) {
    ...
    if (initialOpenSheet) {
      $("#" + Store.container).append(
        replaceHtml(sheetselectlistHTML, { item: item })
      );
      $("#luckysheet-sheet-list").on(
        "click",
